...
parser.add_argument('-n','--nonverbose', help='Do not print all info to console',
                    action='store_true')
parser.add_argument('-s','--samplesheet', help='file with mapping between IDs. If this is used, ID is the ')
...
x = 0
for f in Path(args.cellSNP_dirs).rglob('GT_donors.vireo.vcf.gz'):
    x += 1
    if not args.nonverbose:
        print('Reading '+str(f)+'...')
    with gzip.open(f,'rt') as input_file:
        cellranger_id = str(f).split('/')[-2]
        ...
        for line in input_file:
            ...
            if line[0].startswith('#CHROM'):
                ...
                for donor in donor_IDs:
                    ...
                    donor_genotypes[name+'.'+donor] = []
                    donors.add(name+'.'+donor)
                continue
            ...
            for index, donor in enumerate(line[9:]):
                ...
                genotypes_per_file[chr + '_' + pos][name+'.'+donor_IDs[index]] = genotype
...
with open(args.outdir+'/matched_donors.txt','w') as out:
    ...
    for group in donors_matching:
        ...
        for sample in donors_matching[group]:
            out.write('donor'+str(group_count)+'\t'+sample.split('.')[0]+'\t'+sample.split('.')[1]+'\n')

correlation_dataframe = pandas.DataFrame(correlation_matrix)
correlation_dataframe.columns = donors
correlation_dataframe.index = donors
